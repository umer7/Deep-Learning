...
def compute_content_cost(a_C, a_G):
    ...
    a_C_unrolled = tf.transpose(tf.reshape(a_C,[m,n_H*n_W,n_C]),perm=[0,2,1])
    a_G_unrolled = tf.transpose(tf.reshape(a_C,[m,n_H*n_W,n_C]),perm=[0,2,1])
...
def compute_layer_style_cost(a_S, a_G):
    ...
    J_style_layer = tf.reduce_sum(tf.square(tf.subtract(GG, GS))) / (4 * ((n_H * n_W) ** 2) * (n_C ** 2))
